<#
.SYNOPSIS
    Step 3B-5 - install the F3 parser objects and test F1 + F2 + F3 against the answer key.

.DESCRIPTION
    Requires the Step 3B-4 objects in postgresql_regex_task (answer key table, reference data, output tables,
    validators, f1_candidates, f2_candidates). Connection settings come from PGHOST, PGPORT, PGUSER and
    PGPASSWORD; PGDATABASE is ignored.

    sql/07 and sql/08 are NOT re-run: earlier runs stay in parser_run, and the latest {F1,F2} run (Step 3B-4)
    serves as the F1 + F2 regression baseline for sql/19.

    Order:
      1. check data/expected_fields.csv against data/dataset_manifest.json (SHA-256)
      2. check that the Step 3B-4 objects exist
      3. sql/17_parser_reference_data_f3.sql   re-runnable (F3 key paths)
      4. sql/18_parser_f3.sql                  re-runnable (syslog header + regex-only JSON scan)
      5. sql/20_parser_reference_data_f4.sql   re-runnable (added in Step 3B-6; run_parser needs F4)
      6. sql/21_parser_f4.sql                  re-runnable (added in Step 3B-6)
      7. sql/23_parser_reference_data_f5.sql   re-runnable (added in Step 3B-7)
      8. sql/24_parser_f5.sql                  re-runnable (added in Step 3B-7)
      9. sql/15_parser_core.sql                re-runnable (detect_format, run_parser)
     10. sql/11_parser_evaluation_views.sql    re-runnable
     11. sql/19_test_f3_parser.sql             runs the parser twice and reports the results
...
Write-Host ""
Write-Host "==== 2. Step 3B-4 objects ===="
...
if ("$exists".Trim() -ne 't') { throw "Step 3B-4 objects are missing; run sql\run_step3b4_f2_parser.ps1 first." }
Write-Host "answer key, reference data, output tables, validators, F1 and F2 extractors present"
...
Invoke-Psql -File 'sql/19_test_f3_parser.sql'

Write-Host ""
Write-Host "==== Step 3B-5 F3 parser install and test completed ===="
