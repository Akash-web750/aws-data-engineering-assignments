<#
.SYNOPSIS
    Step 3B-6 - install the F4 parser objects and test F1 - F4 against the answer key.

.DESCRIPTION
    Requires the Step 3B-5 objects in postgresql_regex_task (answer key table, reference data, output tables,
    validators, f1_candidates, f2_candidates, f2_ip_candidates, f3_candidates). Connection settings come from
    PGHOST, PGPORT, PGUSER and PGPASSWORD; PGDATABASE is ignored.

    sql/07 and sql/08 are NOT re-run: earlier runs stay in parser_run, and the latest {F1,F2,F3} run (Step 3B-5)
    serves as the F1 + F2 + F3 regression baseline for sql/22.

    Order:
      1. check data/expected_fields.csv against data/dataset_manifest.json (SHA-256)
      2. check that the Step 3B-5 objects exist
      3. sql/20_parser_reference_data_f4.sql   re-runnable (F4 extras keys)
      4. sql/21_parser_f4.sql                  re-runnable (positional slots, quoted fields, known-key extras)
      5. sql/23_parser_reference_data_f5.sql   re-runnable (added in Step 3B-7; run_parser needs F5)
      6. sql/24_parser_f5.sql                  re-runnable (added in Step 3B-7)
      7. sql/15_parser_core.sql                re-runnable (detect_format, run_parser)
      8. sql/11_parser_evaluation_views.sql    re-runnable
      9. sql/22_test_f4_parser.sql             runs the parser twice and reports the results
...
Write-Host ""
Write-Host "==== 2. Step 3B-5 objects ===="
$check = "SELECT to_regclass('log_regex.expected_fields') IS NOT NULL AND to_regclass('log_regex.ref_key_alias') IS NOT NULL " +
         "AND to_regclass('log_regex.ref_sentinel_phrase') IS NOT NULL AND to_regclass('log_regex.parsed_field') IS NOT NULL " +
         "AND to_regclass('log_regex.parsed_secondary') IS NOT NULL " +
         "AND to_regprocedure('log_regex.f1_candidates(text)') IS NOT NULL " +
         "AND to_regprocedure('log_regex.f2_candidates(text)') IS NOT NULL " +
         "AND to_regprocedure('log_regex.f2_ip_candidates(text,integer,text,text,text)') IS NOT NULL " +
...
if ("$exists".Trim() -ne 't') { throw "Step 3B-5 objects are missing; run sql\run_step3b5_f3_parser.ps1 first." }
Write-Host "answer key, reference data, output tables, validators, F1, F2 and F3 extractors present"
...
Invoke-Psql -File 'sql/22_test_f4_parser.sql'

Write-Host ""
Write-Host "==== Step 3B-6 F4 parser install and test completed ===="
